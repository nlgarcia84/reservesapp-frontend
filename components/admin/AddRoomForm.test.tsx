...
describe('Component AddRoomForm', () => {
  ...
  it('ha de renderitzar el formulari correctament', () => {
    ...
    expect(screen.getByLabelText('Pissarra blanca interactiva')).toBeInTheDocument();
  ...
  it('ha de mostrar un error si s\'intenta enviar sense nom', async () => {
    ...
    expect(await screen.findByText('El nom de la sala és obligatori.')).toBeInTheDocument();
  ...
  const errorMsg = await screen.findByText(/La capacitat ha de ser un número vàlid més gran que 0/i);
  ...
  it('ha de cridar el servei i mostrar èxit si les dades són correctes', async () => {
    (addNewRoom as jest.Mock).mockImplementation(() => Promise.resolve({ id: 1 }));
